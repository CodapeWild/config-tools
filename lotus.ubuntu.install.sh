...
apt install -y mesa-opencl-icd ocl-icd-libopencl1 ocl-icd-opencl-dev ntpdate ubuntu-drivers-common gcc git bzr jq pkg-config curl clang build-essential hwloc libhwloc-dev libhwloc15 wget
...
wget -c https://golang.google.cn/dl/go1.15.5.linux-amd64.tar.gz -O - | sudo tar -xz -C /usr/local
...
cd ~/lotus
git checkout tags/v1.9.0-rc2 -b v1.9.0-rc2
